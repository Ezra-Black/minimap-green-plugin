...
/**
 * Full-game greenscreen overlay.
 *
 * Fills the entire game canvas with the configured chroma-key color and then
 * removes (makes transparent) the triangles that make up the local player's
 * model. This leaves the player visible against a solid background for use
...
public class GreenScreenOverlay extends Overlay
{
	...
	// Constructs the overlay and pins it above the 3D scene.
	@Inject
	...
	public Dimension render(Graphics2D graphics) {
		...
		// Off-screen buffer the size of the client canvas. We draw the greenscreen
		// and cut out the player model here, then blit once to the real canvas.
		...
		// Each polygon is one triangle of the local player's model projected into
		// screen space. Clear all triangles except the "shadow" ones at height 6
		// so only the visible parts of the model are cut out.
	...
	/**
	 * Clears (makes transparent) all pixels contained by the given polygon
	 * on the intermediate image. Used to remove triangles belonging to the
	 * local player's model from the greenscreen buffer.
	 */
